...
const ContentIncomes = () => {
  ...
  e.preventDefault();
  try {
    const response = await axios.post('http://localhost:5000/incomes/new', { // <- nouveau chemin
      amount: parseFloat(formData.amount), // toujours envoyer un nombre
      date: formData.date,
      type: formData.type,
    ...
    // Ajoute directement l’élément créé à ton tableau
  ...
  alert(`Erreur lors de la création du revenu: ${error.response?.data?.message || error.message}`);
}
};
