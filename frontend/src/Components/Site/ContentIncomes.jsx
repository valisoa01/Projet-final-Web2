...
const ContentIncomes = () => {
  ...
  useEffect(() => {
    const fetchIncomes = async () => {
      try {
        ...
        setIncomes(response.data); // Assurez-vous que votre backend renvoie un tableau
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // POST vers la route backend correcte
      const response = await axios.post('http://localhost:5000/api/incomes/test', {
        amount: formData.amount,
        date: formData.date,
        type: formData.type,
        description: formData.description,
        UserId: 1, // à remplacer par l’ID du user connecté
      });

      // Ajouter le revenu renvoyé par le backend à la liste
